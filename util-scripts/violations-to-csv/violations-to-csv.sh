...
output_file="$1"
echo '"Policy", "Description", "Severity", "Cluster", "Namespace", "Deployment", "Time"' > "${output_file}"
...
echo $res | jq -c -r '.alerts[]' | while IFS= read alert; do
  ...
  echo $alert
  echo "====="
  echo "${alert}" | jq -r '[.policy.name, .policy.description, .policy.severity, .deployment.clusterName, .deployment.namespace, .deployment.name, .time] | @csv' >> "${output_file}"
done
